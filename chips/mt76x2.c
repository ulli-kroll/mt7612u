...
void mt76x2_get_current_temp(struct rtmp_adapter *ad)
{
	...
	mt7612u_cfg_read(ad, 0xD000, &temp_val);
	temp_val &= 0x7F;
...
static void patch_BBPL_on(struct rtmp_adapter *pAd)
{
	...
	mt7612u_cfg_read(pAd, 0x130, &value);
	value |= ((1<<16) | (1<<0));
	mt7612u_cfg_write(pAd, 0x130, value);

	udelay(1);

	mt7612u_cfg_read(pAd, 0x64, &value);
	if ((value >> 29) & 0x1) {
		mt7612u_cfg_read(pAd, 0x1c, &value);
		value &= 0xFFFFFF00;
	...
	mt7612u_cfg_read(pAd, 0x14C, &value);
	value  |= ((1<<20) | (1<<19));
...
static void WL_POWER_ON(struct rtmp_adapter *pAd)
{
	...
	mt7612u_cfg_read(pAd, 0x148, &value);
	value |= 0x1;
	mt7612u_cfg_write(pAd, 0x148, value); // turn on WL MTCMOS
	do {
		mt7612u_cfg_read(pAd, 0x148, &regval);
	...
	mt7612u_cfg_read(pAd, 0x1204, &value);
	value &= ~(0x1<<3);
	mt7612u_cfg_write(pAd, 0x1204, value);

	/* WLAN function enable */
	mt7612u_cfg_read(pAd, 0x80, &value);
	value |= (0x1<<0);
